...
export default function Chat() {
  ...
  const [selectedApproach, setSelectedApproach] = useState<'approach1' | 'approach2'>('approach2');
  ...
  const handleNewConversation = async () => {
    try {
      // Clear session from backend if exists
      if (sessionId) {
        try {
          await apiClient.clearSession(sessionId);
        } catch (error) {
          // If session doesn't exist on backend, that's fine - continue with clearing locally
          console.log("Session already cleared or doesn't exist");
        }
      }
      ...
      toast.success("Started new conversation", {
        description: "Previous context cleared successfully",
      });
    } catch (error: any) {
  ...
  const formatResponseContent = (content: any): string => {
    ...
    // If content is an object/array
    if (typeof content === 'object') {
      try {
        // Check if it's a deals response
        if (content.deals && Array.isArray(content.deals)) {
          return content.deals.map((deal: any) => 
            `Deal: ${deal.dealname}\n` +
            `Amount: $${Number(deal.amount).toLocaleString()}\n` +
            `Stage: ${deal.dealstage || 'Not set'}\n` +
            `Close Date: ${deal.closedate ? new Date(deal.closedate).toLocaleDateString() : 'Not set'}\n` +
            `Owner: ${deal.owner || 'Not assigned'}\n`
          ).join('\n---\n');
        }
        
        // Check if it's a channels list response
        if (content.channels && Array.isArray(content.channels)) {
          return 'Available Slack Channels:\n\n' + 
            content.channels.map((channel: any) => 
              `#${channel.name} - ${channel.purpose?.value || 'No purpose set'}`
            ).join('\n');
        }
        
        // For other structured data, try to make it readable
        const formatted = Object.entries(content).map(([key, value]) => {
  ...
  const handleSubmit = async (queryText?: string) => {
    ...
    try {
      const response = await apiClient.processQuery(query, sessionId || undefined, selectedApproach);
      ...
      const formattedResult = formatResponseContent(response.result);
            ...
            <div className="text-center py-12 space-y-8">
              ...
              <div>
                ...
                <p className="text-muted-foreground mb-4">
                  Select an approach and try out some example queries
                </p>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4 max-w-3xl mx-auto mb-8">
                <ApproachCard
                  ...
                  icon="🎯"
                  isSelected={selectedApproach === 'approach1'}
                  onClick={() => setSelectedApproach('approach1')}
                />
                <ApproachCard
                  name="Approach 2"
                  description="Enhanced implementation with advanced agent management"
                  icon="⚡"
                  isSelected={selectedApproach === 'approach2'}
                  onClick={() => setSelectedApproach('approach2')}
                />
              </div>
